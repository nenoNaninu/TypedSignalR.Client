...
public sealed class MethodMetadata
{
    ...
    public MethodMetadata(IMethodSymbol methodSymbol)
    {
        ...
        ReturnType = methodSymbol.ReturnType.ToDisplayString(SymbolDisplayFormatRule.Default);
        ...
        if (returnTypeSymbol is not null)
        {
            ...
            if (returnTypeSymbol.IsGenericType)
            {
                GenericReturnTypeArgument = returnTypeSymbol.TypeArguments.Length == 1
                    ? returnTypeSymbol.TypeArguments[0].ToDisplayString(SymbolDisplayFormatRule.Default)
                    : string.Join(", ", returnTypeSymbol.TypeArguments.Select(x => x.ToDisplayString(SymbolDisplayFormatRule.Default)));
            }
        }
    }
}
